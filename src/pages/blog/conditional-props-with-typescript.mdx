...
const Button = ({ children, as, type, onClick, href }: Props): JSX.Element => {
    const styles = "py-2 px-6 bg-blue-600 text-white rounded";
...
const Button = ({ children, as: El, ...restProps }: Props): JSX.Element => (
    <El className="py-2 px-6 bg-blue-600 text-white rounded" {...restProps}>
        {children}
    </El>
);
```
...
Generally speaking I think it's good wherever possible to push data validation
like this to the TypeScript level rather than runtime level. Not only is
TypeScript a really good tool for this, but it means you're runtime code is
simpler and you ship less JavaScript to the browser. It's also a far more direct
and efficient developer experience to get errors in your IDE than in the
browser.
