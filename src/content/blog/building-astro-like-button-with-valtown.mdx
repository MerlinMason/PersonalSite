...
Astro is the framework this blog is build on, it's a static site generator that
implements the
...
-   We want to display the count of likes per post
-   We want to enable a reader to like a post without having to authenticate
-   We want to prevent a reader from liking a post more than once
-   We want to take reasonable steps to prevent from spam and abuse
...
The formatting currently removes line breaks which a find impacts the
